...
"""You are organizing a series of events, and each event is represented by a parenthesis in the string schedule,
 where an opening parenthesis ( represents the start of an event, and a closing parenthesis ) represents the end of an event. A balanced schedule means every event that starts has a corresponding end.
...
def min_changes_to_make_balanced(schedule):
    ...
    for paren in schedule: #( ,
        # print("curr stack",stack)
        # print("Current Paren: ",paren)
        # if stack and paren[close] in stack

        if paren in close:
            if stack and close[paren] == stack[-1]:
                stack.pop()
            else:
                stack.append(paren)
        else:
